    ...
    <div class="pingLun">
        ...
        <div class="pingLinInputBox" v-if="$store.state.user">
            <el-input
                type="textarea"
                :rows="5"
                placeholder="请输入内容,最少10个字，最多200字"
                @change="change"
                v-model="from.pingLun">
            </el-input>
            <div class="tar mt10">
                <mu-button @click="cancel" v-if="!hide_cancel">取消</mu-button>
                <mu-button color="success" @click="success" :disabled="from.pingLun && from.pingLun.length>=10 && from.pingLun.length<=200?false:true">发送</mu-button>
            </div>
        </div>
